...
async function sendServiceRequestNotification(serviceRequest) {
  const fromAddress = process.env.SMTP_FROM || `${process.env.BUSINESS_NAME || 'Private ITIN Assistance Service'} <no-reply@example.com>`;
  const toAddress = process.env.CONTACT_NOTIFICATION_TO || process.env.SUPPORT_EMAIL || 'contact@etaxids.com';
  const subject = `New ITIN service request: ${serviceRequest.serviceLabel}`;
  const text = [
    'A new ITIN service request was submitted on the website.',
    '',
    `Requested service: ${serviceRequest.serviceLabel}`,
    `Name: ${serviceRequest.firstName} ${serviceRequest.lastName}`.trim(),
    `Email: ${serviceRequest.email}`,
    `Phone: ${serviceRequest.phone}`,
    `Current ITIN / Tracking Code: ${serviceRequest.referenceNumber || 'Not provided'}`,
    '',
    'Request details:',
    serviceRequest.message || 'No additional details provided.',
  ].join('\n');
